...
class FlexReporter:
    @staticmethod
    def run_report(token=None, query_id=None,
                   local_date=None, report_type="Daily"):
        token = token or settings.IB_FLEX_TOKEN
        query_id = query_id or settings.IB_FLEX_DAILY_QUERY_ID

        html = []  # String builder — replaces print() + sys.stdout redirect
        telegram_msgs = []
        dividend_msg = ""
        has_dividends = False
        ...
        try:
            ...
            if local_date:
                try:
                    ...
                    file_path = os.path.join(
                        dirname, f'../flex_queries/{local_date}.xml')
                    with open(file_path, 'rb') as f:
                        response = f.read()
                    html.append(f"<!-- Loaded local report: {local_date}.xml -->")
                ...
                try:
                    ...
                    with open(full_path, 'w') as f:
                        f.write(response.decode('utf-8')
                                if isinstance(response, bytes) else response)
                    archive_status = f"Saved: {arch_filename}"
                except Exception as e:
                    archive_status = f"Failed: {e}"
                    html.append(f"<!-- Archiving failed: {e} -->")
            else:
            ...
            for cashReport in root.iter('CashReportCurrency'):
                if cashReport.get('currency') == 'SEK':
                    continue
                idx += 1
                c = cashReport.get
                ...
                html.append(f'<tr class="{"even" if idx % 2 else "odd"}">')
                html.append(f'<td class="c">{cur_display}</td>')
                html.append(f'<td class="r">{fmt_num(c("startingCash"))}</td>')
                html.append(f'<td class="r">{fmt_num(c("endingCash"))}</td>')
                html.append(f'<td class="r">{fmt_num(c("endingSettledCash"))}</td>')
                html.append(f'<td class="r">{fmt_num(c("deposits"))}</td>')
                html.append(f'<td class="r">{fmt_num(c("withdrawals"))}</td>')
                html.append(f'<td class="r">{fmt_num(c("netTradesPurchases"))}</td>')
                html.append(f'<td class="r">{fmt_num(c("netTradesSales"))}</td>')
                html.append(f'<td class="r">{fmt_num(c("dividends"))}</td>')
                html.append(f'<td class="r">{fmt_num(c("paymentInLieu"))}</td>')
                html.append(f'<td class="r">{fmt_num(c("withholdingTax"))}</td>')
                html.append(f'<td class="r">{fmt_num(c("brokerInterest"))}</td>')
                html.append(f'<td class="r">{fmt_num(c("commissions"))}</td>')
            ...
            # CashTransactions
            html.append('<h2>Cash Transactions</h2>')
            ...
            for i, attrs in enumerate(cash_txs):
                ...
                html.append(f'<td class="r">{a("symbol")}</td><td class="c">{a("dateTime").split(";", 1)[0]}</td><td class="c">{a("currency")}</td>')
                html.append(f'<td>{fmt_num(a("fxRateToBase"), 7)}</td>')
                html.append(f'<td class="r {cls}">{fmt_num(a("amount"), 4)}</td><td class="c">{a("type")}</td>')
                html.append(f'<td>{a("description").replace(a("symbol"), "").strip()}</td><td class="r">{a("listingExchange")}</td>')
                html.append('</tr>')
            html.append('</tbody></table>')
            if has_dividends:
            ...
            html.append('<table> <thead> <tr><td>tckr</td> <td>date</td> <td>cur</td> <td>fxRate</td> <td>tckr desc.</td> <td>taxAmt</td> <td>description</td> <td>exchg</td></tr> </thead> <tbody>')
            taxes = list(root.iter('TransactionTax'))
            sortchildrenby(taxes, 'date')
            for i, a in enumerate(taxes):
                html.append(f'<tr class="{"even" if i % 2 else "odd"}">')
                html.append(f'<td>{a.get("symbol")}</td><td>{a.get("date")}</td><td>{a.get("currency")}</td><td>{fmt_num(a.get("fxRateToBase"), 7)}</td>')
                html.append(f'<td>{a.get("description")}</td><td>{fmt_num(a.get("taxAmount"), 5)}</td><td>{a.get("taxDescription")}</td>')
                html.append(f'<td class="r">{a.get("listingExchange")}</td></tr>')
            html.append('</tbody></table>')

            # ChangeInDividendAccruals
            html.append('<h2>Change in Dividend Accruals</h2>')
            html.append('<table> <thead> <tr><td>tckr</td> <td>exdate</td> <td>paydate</td> <td>cur</td> <td>qty</td> <td>gRate</td> <td>gAmt</td> <td>tax</td> <td>taxPct</td> <td>nAmt</td> <td>description</td> <td>exchg</td></tr> </thead> <tbody>')
            ...
            for a in accruals:
                ...
                taxpct = (tax * 100) / \
                    (qty * grate) if (qty * grate) != 0 else 0

                html.append(f'<tr class="{cls}">')
                html.append(f'<td class="r">{at("symbol").rjust(4)}</td><td>{at("exDate")}</td><td>{at("payDate")}</td><td>{at("currency")}</td>')
                html.append(f'<td class="r">{fmt_num(at("quantity"), 0).rjust(3)}</td><td class="r">{fmt_num(at("grossRate"), 3)}</td><td class="r">{fmt_num(at("grossAmount"), 3)}</td>')
                html.append(f'<td class="r">{fmt_num(at("tax"), 3)}</td><td class="r">{fmt_num(taxpct, 2)}%</td><td class="r">{fmt_num(at("netAmount"), 3)}</td>')
                html.append(f'<td>{at("description")}</td><td class="r">{at("listingExchange")}</td></tr>')
            html.append('</tbody></table>')

            # OpenDividendAccruals
            html.append('<h2>Open Dividend Accruals</h2>')
            html.append('<table> <thead> <tr><td>tckr</td> <td>exdate</td> <td>paydate</td> <td>cur</td> <td>qty</td> <td>gRate</td> <td>gAmt</td> <td>tax</td> <td>taxPct</td> <td>nAmt</td> <td>description</td> <td>exchg</td></tr> </thead> <tbody>')
            ...
            # TierInterestDetails
            html.append('<h2>Interest Details</h2>')
            html.append('<table> <thead> <tr><td>date</td> <td>cur</td> <td>total</td> <td>fxRate</td> <td>rate</td> <td>amt</td> <td>description</td></tr> </thead> <tbody>')
            interest = list(root.iter('TierInterestDetail'))
            sortchildrenby(interest, 'valueDate', 'currency')
            for i, a in enumerate(interest):
                at = a.get
                html.append(f'<tr class="{"even" if i % 2 else "odd"}">')
                html.append(f'<td>{at("valueDate")}</td><td>{at("currency")}</td><td class="r">{fmt_num(at("totalPrincipal"))}</td>')
                html.append(f'<td>{fmt_num(at("fxRateToBase"), 7)}</td><td class="r">{fmt_num(at("rate"))}%</td><td>{fmt_num(at("totalInterest"))}</td><td>{at("interestType")}</td></tr>')
            html.append('</tbody></table>')

            # Trades
            html.append('<h2>Trades</h2>')
            html.append('<table> <thead> <tr><td>tckr</td> <td>date</td> <td>buySell</td> <td>cur</td> <td>fxRate</td> <td>qty</td> <td>price</td> <td>comm</td> <td>comCur</td> <td>netCash</td> <td>desc</td> <td>underl</td> <td>mult</td> <td>strike</td> <td>expiry</td> <td>p/c</td> <td>exchg</td> <td>listExch</td> <td>undExh</td></tr> </thead> <tbody>')
            ...
            for a in trades:
                ...
                html.append(f'<tr class="{cls}">')
                html.append(f'<td class="r">{at("symbol")}</td><td>{at("tradeDate")}</td><td class="c">{at("buySell")}</td><td>{at("currency")}</td>')
                html.append(f'<td>{fmt_num(at("fxRateToBase"), 7)}</td><td class="r">{fmt_num(at("quantity"))}</td><td class="r">{fmt_num(u_price, 4)}</td>')
                html.append(f'<td class="r">{fmt_num(-1 * comm, 8)}</td><td class="c">{at("ibCommissionCurrency")}</td><td class="r">{fmt_num(at("netCash"), 8)}</td>')
                html.append(f'<td>{at("description")}</td><td class="r">{at("underlyingSymbol")}</td><td class="r">{fmt_num(at("multiplier"))}</td>')
                html.append(f'<td class="r">{fmt_num(at("strike"), 2) if at("strike") else ""}</td><td>{at("expiry")}</td><td class="c">{at("putCall")}</td>')
                html.append(f'<td class="c">{at("exchange")}</td><td class="c">{at("listingExchange")}</td><td class="c">{at("underlyingListingExchange")}</td></tr>')
            html.append('</tbody></table>')

            # ConversionRates
            html.append('<h2>Conversion Rates</h2>')
            html.append('<table> <thead> <tr><td>date</td> <td>from</td> <td>rate</td> <td>to</td> <td>rate</td></tr> </thead> <tbody>')
            rates = list(root.iter('ConversionRate'))
            sortchildrenby(rates, 'reportDate', 'fromCurrency')
            idx = 0
            for a in rates:
                at = a.get
                if at('fromCurrency') not in ['USD', 'GBP']:
                    continue
                idx += 1
                rate = float(at('rate') or 1)
                inv = 1 / rate if rate != 0 else 0
                html.append(f'<tr class="{"even" if idx % 2 else "odd"}">')
                html.append(f'<td>{at("reportDate")}</td><td>{at("fromCurrency")}EUR</td><td>{fmt_num(rate, 7)}</td>')
                html.append(f'<td>EUR{at("fromCurrency")}</td><td>{fmt_num(inv, 10)}</td></tr>')
            html.append('</tbody></table>')
