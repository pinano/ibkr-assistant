        ...
        try:
            ...
            if local_date:
                try:
                    ...
                    # Assuming file format YYYYMMDD.xml (user requested
                    # YYYYMMDD arg)
            ...
            if not local_date:
                try:
                    ...
                    # User requested YYYYMMDD.xml format, add -monthly suffix
                    # if applicable
            ...
            for a in accruals:
                ...
                print(
                    f'<td>{at("description")}</td><td class="r">{at("listingExchange")}</td></tr>')
            print('</tbody></table>')
            ...
            for a in rates:
                ...
                print(f'<tr class="{"even" if idx % 2 else "odd"}">')
                print(
                    f'<td>{at("reportDate")}</td><td>{at("fromCurrency")}EUR</td><td>{fmt_num(rate, 7)}</td>')
                print(
                    f'<td>EUR{at("fromCurrency")}</td><td>{fmt_num(inv, 10)}</td></tr>')
            print('</tbody></table>')
